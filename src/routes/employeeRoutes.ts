...
router.get("/:id/personal", async (req: Request, res: Response) => {
  const [rows]: any = await pool.query(
    "SELECT * FROM personal_details WHERE candidate_id = ?",
    [req.params.id]
  );
  res.json({ candidateId: req.params.id, personalDetails: rows[0] || {} });
});

router.get("/:id/job", async (req: Request, res: Response) => {
  const [rows]: any = await pool.query(
    "SELECT * FROM job_details WHERE candidate_id = ?",
    [req.params.id]
  );
  res.json({ candidateId: req.params.id, jobDetailsForm: rows[0] || {} });
});

router.get("/:id/offer", async (req: Request, res: Response) => {
  const [rows]: any = await pool.query(
    "SELECT * FROM offer_details WHERE candidate_id = ?",
    [req.params.id]
  );
...
router.get("/:id/credentials", async (req: Request, res: Response) => {
  const [rows]: any = await pool.query(
    "SELECT * FROM employee_credentials WHERE candidate_id = ?",
    [req.params.id]
  );
